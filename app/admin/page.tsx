...
function getPumpProgress(pump?: Pump): number {
  if (!pump) return 0;
  const start = new Date(pump.startTime).getTime();
  const end = new Date(pump.endTime).getTime();
  const now = Date.now();
...
function getPumpTimeRemaining(pump?: Pump): string {
  if (!pump) return '-';
  const end = new Date(pump.endTime).getTime();
...
export default function AdminPage() {
    ...
    async function loadAdmin() {
      try {
        ...
        if (usersRes.ok) {
          ...
          const pumpRes = await fetch('/api/admin/pumps');
          if (pumpRes.ok) {
  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <Card>
          ...
          <CardContent>
            <div className="md:hidden space-y-3 mb-4">
              {users.map((user) => (
                <Card key={`m-${user.id}`} className="border">
                  <CardContent className="py-4 space-y-3">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="font-semibold">{user.username}</p>
                        <p className="text-xs text-muted-foreground">{user.email}</p>
                        <p className="text-xs text-muted-foreground mt-1">{user.isAdmin ? 'Admin' : 'User'} • {new Date(user.createdAt).toLocaleDateString()}</p>
                      </div>
                      {hasActivePump(user.id) && (
                        <span className="px-2 py-1 rounded-full text-[10px] font-bold bg-primary/20 text-primary">LIVE PUMP</span>
                      )}
                    </div>
                    <div className="text-sm space-y-1">
                      <p className="font-medium">Cash: {portfolios[user.id] ? formatCurrency(portfolios[user.id].cash) : '$0'}</p>
                      {hasActivePump(user.id) && activePumps[user.id] && (
                        <p className="text-xs text-primary font-medium">
                          Progress: {getPumpProgress(activePumps[user.id]).toFixed(1)}% • Remaining: {getPumpTimeRemaining(activePumps[user.id])}
                        </p>
                      )}
                    </div>
                    <div className="flex flex-wrap gap-2">
                      {hasActivePump(user.id) ? (
                        <Button size="sm" variant="destructive" onClick={() => handleCancelPump(user.id)}>Cancel Pump</Button>
                      ) : (
                        <Button size="sm" variant="outline" onClick={() => openPumpDialog(user.id)} disabled={!canPump(user.id)}>
                          <Zap className="w-4 h-4" />
                        </Button>
                      )}
                      <Button size="sm" variant="outline" onClick={() => openBalanceDialog(user.id)}>
                        <DollarSign className="w-4 h-4" />
                      </Button>
                      <Button size="sm" variant="destructive" onClick={() => setDeleteUserId(user.id)} disabled={user.isAdmin}>
                          ...
                          <div>
                            ...
                            {hasActivePump(user.id) && activePumps[user.id] && (
                              <p className="text-xs text-primary font-medium mt-1">
                                {getPumpProgress(activePumps[user.id]).toFixed(1)}% • {getPumpTimeRemaining(activePumps[user.id])} left
                              </p>
                            )}
                          </div>
                        </div>
      ...
      <MobileBottomNav current="admin" isAdmin />
